...
public class Ex {
	...
	public static void printT(int h, int w)
	{
		if(w%2==0 || w>h*2)
		...
		{
			   int oddNums=((w-1)/2)-1;
			   int lNum=(oddNums==0)?0:((h-2)/oddNums);
			  
			   int rest=(oddNums==0)?h-2:(h-2)%oddNums;
	...
	{ 
		...
		{
			int scope=(height*2)+(width*2);
			System.out.println("The rectangle's scope is:"+ scope);
		}
